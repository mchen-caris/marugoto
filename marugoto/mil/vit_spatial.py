...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv(x)  # Apply convolution: (batch_size, 1, height, width)
        x = x.squeeze(1)  # Remove channel dimension: (batch_size, height, width)
        x = self.sigmoid(x)  # Apply sigmoid to obtain attention map: (batch_size, height, width)
        return x
...
class MILModel(nn.Module):
    def __init__(self, n_classes, input_dim=1024, attention_dim=64, d_model=512, nhead=8, num_layers=6):
        ...
        self.linear = nn.Linear(64 * 32 * conv1_output_dim, d_model)
    ...
    def forward(self, x, register_hook=False):
        ...
        # Spatial attention
        spatial_attention_weights = self.spatial_attention(x)
        x = x * spatial_attention_weights.unsqueeze(2)
